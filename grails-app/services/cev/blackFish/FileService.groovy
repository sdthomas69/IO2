...
class FileService {
	...
	void createThumbnails(String filePath) {
		...
		String arg = """-thumbnail ${config.smIconWidth}X${config.smIconHeight}^ -gravity center -crop ${config.smIconWidth}X${config.smIconHeight}-0-0"""
		createThumbnail(filePath, "_smsq.jpg", arg)
		
		arg = "-thumbnail " + config.smIconWidth.toString()
		createThumbnail(filePath, "_sm.jpg", arg)

		arg = """-thumbnail ${config.tinyIconWidth}X${config.tinyIconHeight}^ -gravity center -crop ${config.tinyIconWidth}X${config.tinyIconHeight}-0-0"""
		createThumbnail(filePath, "_smaller.jpg", arg)
		
		arg = """-thumbnail ${config.smallSlideWidth}X${config.smallSlideHeight}^ -gravity center -crop ${config.smallSlideWidth}X${config.smallSlideHeight}-0-0"""
		createThumbnail(filePath, "_sm_slide.jpg", arg)
		
		arg = """-thumbnail ${config.slideWidth}X${config.slideHeight}^ -gravity center -crop ${config.slideWidth}X${config.slideHeight}-0-0"""
