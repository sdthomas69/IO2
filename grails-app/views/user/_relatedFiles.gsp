	...
	<div id="story_highlights">
		...
		<div id="tinyThumbs100">
			<g:each in="${shiroUser.files?.sort()}" var="file" status="i">
				<div class='<blackFish:setClass counter="${i}" width="4" beginDiv="grid_1 alpha" middleDiv="grid_1" endDiv="grid_1 omega" />'>
					<g:link action="show" controller="file" id="${file.id}">
						<img src="<g:resource file='${file.tinyImage ?: file.smallImage}' />" class="imageborder" alt="${file}" width="36" height="36" border="0">
					</g:link>
				</div>
				<g:if test="${i % 4 == 3}">
