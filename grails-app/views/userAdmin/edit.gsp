        ...
        <div class="form-block">
            <div data-duration-in="300" data-duration-out="100" class="w-tabs admin-text-tabs">
                <div class="w-tab-menu admin-tabs-menu-container">
                    <a data-w-tab="Personal"
                       class="w-tab-link ${flash.tab == null ? 'w--current' : ''} w-inline-block tab-link-button">
                        <div class="intranet-text-block">Personal Information</div>
                    </a>
                    <a data-w-tab="Files"
                       class="w-tab-link ${flash.tab == 'files' ? 'w--current' : ''} w-inline-block tab-link-button">
                        <div>Files</div>
                    </a>
                    <a data-w-tab="Stories"
                       class="w-tab-link ${flash.tab == 'stories' ? 'w--current' : ''} w-inline-block tab-link-button">
                        <div>Stories</div>
                    </a>
                ...
                <div class="w-tab-content admin-tabs-container">
                    ...
                    <div data-w-tab="Stories" class="w-tab-pane ${flash.tab == 'stories' ? 'w--tab-active' : ''}">
                        ...
                        <g:link class="w-button submit-button" controller="storyAdmin"
                                params="['author.id': userInstance?.id]" action="createText">
                            +&nbsp;Add A New Story
                        </g:link>
                    ...
                    <div data-w-tab="Files" class="w-tab-pane ${flash.tab == 'files' ? 'w--tab-active' : ''}">
                        <g:link class="w-button submit-button" controller="fileAdmin"
                                params="['author.id': userInstance?.id]" action="create">
                            +&nbsp;Add A New File
                        </g:link>
