				...
				<div id="storyList">
					...
					<div class="w-section w-clearfix grid" id="grid">
						...
						<g:each in="${userInstanceList}" var="user" status="i">
							<div class="item">
								<g:set var="link" value="${createLink(controller: 'user', action: 'home', params: ['firstAndLast' : user?.name])}" />
								<a class="overlay2" href="${link}"> 
									<div class="description">
										${user?.firstName} ${user?.lastName}
									</div>
									<img class="image"src="<g:resource file='${user?.primaryImage?.smallImageSquared ?: thumbnail }' />" alt="${user}" />
								</a>
							</div>
						</g:each>
